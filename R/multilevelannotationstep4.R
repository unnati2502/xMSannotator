multilevelannotationstep4 <- function(outloc,
    ...
    if (FALSE) {
        ...
        chemscoremat_highconf$Adduct <-
            gsub(chemscoremat_highconf$Adduct,
                 pattern = "[\\\"']",
                 replacement = "")
    }
    ...
    # if(length(chemids)>1000)
    {
        ...
        # for(i in s1)
        {
            # if(FALSE)
            {
                ...
                clusterExport(cl, "get_confidence_stage4",
                              envir = environment())
                ...
                clusterExport(cl, "chemscoremat_highconf",
                              envir = environment())
    ...
    if (FALSE) {
        if (length(isp_ind) > 0) {
            ...
            curated_res$Adduct[isp_ind] <-
                as.character(temp_adducts)
            # print(curated_res$Adduct[isp_ind])
        }
    }
